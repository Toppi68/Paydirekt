...
require_once (_INCLUDEPATH.'/Utility/responseMessages.cls.php');

class communication extends responseMessages {
    ...
    private function getToken() {
        ...
        //Anfordern
        require_once (_INCLUDEPATH.'/Data/gvToken.cls.php');
        $token = new gvToken();
        $req = $token->send();
    ...
    private function sendCurl ($url="", $arrHeader='', $postVar='') {
		...
		if(file_exists(_INCLUDEPATH.'/Cert/paydirekt.de.pem')){
			curl_setopt($request, CURLOPT_CAINFO, _INCLUDEPATH.'/Cert/paydirekt.de.pem');
		}
        if($arrHeader) {
